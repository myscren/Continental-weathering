...
data=zeros(length(K_AL),length(simitems));
...
test=~isnan(K_AL(:,1)); % No point resampling by age if there isn't age data
data=data(test,:);
uncertainty=uncertainty(test,:);

figure;
scatter(K_AL(:,1),K_AL(:,2),6,'MarkerEdgeColor','b');
hold on
scatter(data(:,1),data(:,2),6,'MarkerEdgeColor','r');

tic;
fprintf('Calculating sample weights: ')
    k=invweightAge(K_AL(:,1));
    %U.k=k;
% end
...
[c,m,e]=bin(mczircon.Age(test),mczircon.(Elem)(test),agemin,agemax,length(mczircon.Age)./length(K_AL(:,1)),nbins,binoverlap);
